...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        if data_multiple is None:
            ...
            list_data_dict = [
                item for dataset_i in list_data_dict for item in dataset_i
            ]
            ...
            for data_scaler_i, dataset_i in zip(data_multiple, list_data_dict):
                ...
                logging.warning(
                    f"Multiplying {dataset_name_i} by {data_scaler_i} times"
                )
    ...
    def sample_point_in_segment(
        self, mask, ratio_w, ratio_h, box=None, sampling="uniform"
    ):
        ...
        if sampling == "center" or sampling == "gaussian":
            if sampling == "center":
                ...
                box_anchor_w, box_anchor_h = self.get_obj_center(
                    box, 1, 1, std_dev_weight=0.15
                )
            # get 1000 random items from the indices
            sampled_list = random.sample(
                list(range(len(indices))), min(1000, len(indices))
            )
            min_dis = 1e6
            min_point = None
            for sampled_i in sampled_list:
                point_i = indices[sampled_i]
                dis_i = (point_i[0] - box_anchor_h) ** 2 + (
                    point_i[1] - box_anchor_w
                ) ** 2
    ...
    def generate_mask_for_feature(self, coor, box, mask, raw_w, raw_h):
        ...
        if len(coor) == 2:
            ...
            coor_mask = (
                coor_mask * sam_mask if sam_mask is not None else coor_mask * box_mask
            )
            assert (
                coor_mask == 1
            ).any(), f"coor: {coor}, box: {box}, raw_w: {raw_w}, raw_h: {raw_h}"
        elif len(coor) == 4:
            coor_mask = box_mask * sam_mask if sam_mask is not None else box_mask
            if (coor_mask == 0).all():
                print(
                    "Find one sample sam mask and box has no overlap, use box mask only"
                )
                coor_mask = box_mask
            assert (
                coor_mask == 1
                    ...
                    for box_idx, box_i in enumerate(box_list_i):
                            ...
                            if (
                                ...
                                if "segment_mask" in self.point_input_sample:
                                    if "masks" in sources[0]:
                                        ...
                                        assert (
                                            cur_mask["size"][0] == sources[0]["image_h"]
                                        )
                                        assert (
                                            cur_mask["size"][1] == sources[0]["image_w"]
                                        )
                                        if (
                                            "uniform"
                                            in self.point_input_sample.split("|")[1]
                                        ):
                        ...
                        if self.add_region_feature and box_list_idx % 2 == 0:
                            ...
                            if self.no_coor:
                                ...
                                cur_conv = cur_conv.replace(
                                    f"<bbox_location{box_idx}>", coor_i
                                )
